...
/**
 * <!-- globalinfo-start --> Implements the Haversine distance.<br/>
 * <br/>
 ...
 * @version 1.0
 */
public class HaversineDistance extends NormalizableDistance implements
	...
	/**
	 * Constructs a Haversine Distance object, Instances must be still set.
	 */
	...
	/**
	 * Constructs a Haversine Distance object and automatically initializes the
	 * ranges.
	 * 
	 * @param data
	 *            the instances the distance function should work on
	 */
	public HaversineDistance(Instances data) {
		super(data);
	}
	...
	public String globalInfo() {
		return "Implements the Haversine distance. \n\n"
	 ...
	 * 
	 */
	private static double distHaversine(double lat1, double lon1, double lat2,
			double lon2) {
		...
		double sindLon = Math.sin(dLng / 2.0);

		double a = Math.pow(sindLat, 2)
				+ (Math.pow(sindLon, 2) * Math.cos(lat1) * Math.cos(lat2));
	...
	public double distance(Instance first, Instance second, double limit,
		...
		return distHaversine(paramOne, paramTwo, paramThree, paramFour);
